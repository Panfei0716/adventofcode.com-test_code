...
/* replace position 1 with the value and replace position 2 with the value.*/
	    contain_init(noun, verb, contain);
	    debug
		("contain[1]=%d,contain[2]=%d,noun=%d, verb=%d\n",
		 contain[1], contain[2], noun, verb);
		...
		case 1:
		    ...
		    debug
			("for1 contain=%d,i=%d,ret=%d\n",
			 contain[i], i, ret);
		    break;
		case 2:
		    ret = multiplication(i, contain);
		    debug
			("for2 contain=%d,i=%d,ret=%d\n",
			 contain[i], i, ret);
		    break;
		case 99:
		    debug
			("the program is finished contain[0]=%d\n",
			 contain[0]);
		    //      look_all_contain();
		    if (contain[0] == 19690720) {
			printf
			    ("the program is finished 100*noun(%d)+verb(%d)=%d\n",
